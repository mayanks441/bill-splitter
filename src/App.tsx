...
const AVATAR_BG: Record<string, string> = { Mayank: "#6366f1", Shantanu: "#10b981", Anuj: "#f59e0b" };

const n = (v: string | number) => Math.max(0, parseFloat(String(v)) || 0);
const used = (p: string, c: string) => Math.max(0, n(c) - n(p));
...
const init = () => ({
  ...
  recharge: { Mayank: "", Shantanu: "", Anuj: "" } as Record<string, string>,
});
...
const lbl: React.CSSProperties = { fontSize: 11, color: "rgba(255,255,255,0.45)", marginBottom: 6, letterSpacing: "0.05em", textTransform: "uppercase" };

export default function App() {
  const [inp, setInp] = useState(init());
  const [result, setResult] = useState<null | ReturnType<typeof computeResult>>(null);
  const [err, setErr] = useState("");

  const acUnits = (nm: string) => used(inp.ac[nm].prev, inp.ac[nm].curr);
  ...
  const setAC = (nm: string, field: string, val: string) =>
    setInp(p => ({ ...p, ac: { ...p.ac, [nm]: { ...p.ac[nm], [field]: val } } }));
  const setR = (nm: string, val: string) =>
  ...
  function computeResult() {
    ...
    const data: Record<string, { ac: number; acCost: number; commonCost: number; fixedShare: number; payable: number; paid: number; bal: number }> = {};
    NAMES.forEach(nm => {
      const ac = acUnits(nm);
    ...
    return { data, txns, rate, commonUnits, fixedShare };
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f0c29 0%, #1a1a4e 40%, #24243e 100%)", padding: "32px 18px 56px", fontFamily: "system-ui, sans-serif" }}>
